...
import { Trophy, Star, ThumbsUp, Award, CheckCircle2, Store, ArrowRight } from 'lucide-react';
...
export default function FeaturedBrandShowcase() {
  ...
  const fetchData = async () => {
    try {
      ...
      if (showRes?.data?.showcase) {
        setShowcase(showRes.data.showcase);
      }
      if (Array.isArray(openRes?.data?.competitions)) {
        setOpenCompetitions(openRes.data.competitions);
      } else if (Array.isArray(openRes?.data?.data)) {
        setOpenCompetitions(openRes.data.data);
      } else {
        setOpenCompetitions([]);
      }
    } catch (error) {
  ...
  const comps = Array.isArray(openCompetitions) ? openCompetitions : [];
  const activeVotingComps = comps.filter(c => c && c.applicants && c.applicants.length > 0 && c.status === 'open');
